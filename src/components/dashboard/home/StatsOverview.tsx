...
export function StatsOverview({ stats }: StatsOverviewProps) {
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold text-gray-900 mb-4">Panoramica</h2>
        ...
        {statCards.map((card) => {
          ...
          return (
            ...
            >
              ...
              {/* Label */}
              <div className="text-sm font-medium text-gray-900 mb-1">{card.label}</div>
      ...
      <div className="mt-6 bg-white rounded-lg border border-gray-200 p-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <StatBadge label="Aperti alle iscrizioni" value={stats.open} color="green" />
          <StatBadge label="In pianificazione" value={stats.planning} color="blue" />
          <StatBadge label="Bozze" value={stats.draft} color="gray" />
          {stats.cancelled > 0 && (
            <StatBadge label="Annullati" value={stats.cancelled} color="red" />
          )}
        </div>
      </div>
    </div>
